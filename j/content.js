...
let NextAction = "a8019ee521f9381ad1570d0211a08fcc53e45ecd"

function NextTree(id, url, IdentifierId) {
  const tree = [
    "",
    {
      "children": [
          ...
          "children": [
            "chat",
            {
              "children": [
                ...
                {
                  "children": [
                    `__PAGE__?{"id":"${IdentifierId}","lang":"en","character-id":"${id}"}`,
                    {},
                    url,
                    "refresh"
                  ]
...
setTimeout(() => {
    ...
    floatingUI.style.pointerEvents = "auto";
}, 50);
...
floatingUI.dataset.expanded = "true";
      ...
      const Popup = {
        init: function () {
            ...
            fontLink.href = 'https://fonts.googleapis.com/css2?family=Baloo+Da+2:wght@400;600&display=swap';
            fontLink.rel = 'stylesheet';
            document.head.appendChild(fontLink);
    
             if (sessionStorage.getItem('popupShown') || sessionStorage.getItem('popupDismissed')) return;
        ...
        createPopup: function () {
            ...
            closeButton.addEventListener('mouseleave', () => closeButton.style.color = '#bbb');
            ...
            popupIcon.innerHTML = `
                <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#ffcc00" stroke-width="2.5"
            ...
            popupHeader.innerText = 'Notification';
            ...
            headerContainer.appendChild(popupIcon);
            headerContainer.appendChild(popupHeader);
    
            const popupMessage = document.createElement('div');
            popupMessage.innerText = 'Join The Discord Server Now~! PS: This would make announcement much easier~! So please Join!';
            ...
            discordButton.innerText = 'Join Discord';
            ...
            discordButton.addEventListener('mouseenter', () => {
                discordButton.style.boxShadow = '0 0 20px rgba(88, 101, 242, 0.8)';
                discordButton.style.transform = 'scale(1.05)';
            });
    
            discordButton.addEventListener('mouseleave', () => {
                discordButton.style.boxShadow = '0 0 10px rgba(88, 101, 242, 0.5)';
                discordButton.style.transform = 'scale(1)';
            });
    
            discordButton.addEventListener('click', () => {
                window.location.href = 'https://discord.gg/phxtgqkQ';
            });
    
            this.popupContainer.appendChild(closeButton);
            this.popupContainer.appendChild(headerContainer);
            this.popupContainer.appendChild(popupMessage);
            this.popupContainer.appendChild(discordButton);
        },
        ...
        dismiss: function () {
            ...
            this.popupContainer.style.transform = 'translate(-50%, -50%) scale(0.9)';
            setTimeout(() => document.body.removeChild(this.popupContainer), 300);
            sessionStorage.setItem('popupDismissed', 'true');
        }
    };
    
    if (!sessionStorage.getItem('popupShown') && !sessionStorage.getItem('popupDismissed')) {
      Popup.init();
      Popup.show();
  }
...
<div class="support-popup">
  ...
  `;
              ...
              console.log("User is VIP, no popup needed.");
          }
      })
      .catch(error => {
      ...
      const createOverlay = (title, contentElements) => {
        ...
        closeButton.addEventListener("click", () => {
            overlayUI.style.transform = "translateY(20px)";
        ...
        overlayUI.appendChild(contentContainer);
        overlay.appendChild(overlayUI);
        document.body.appendChild(overlay);
    
        requestAnimationFrame(() => {
            overlay.style.opacity = "1";
            overlayUI.style.transform = "translateY(0)";
        ...
        tokenizeButton.onclick = async () => {
          ...
          try {
            ...
            if (!loginCheckResponse.ok) {
              throw new Error(`Error ${loginCheckResponse.status}: ${data.message || "Unknown error"}`);
            }
          ...
          textareas.forEach((textarea) => {
            ...
            const processText = async () => {
              ...
              typingTimeout = setTimeout(async () => {
                ...
                try {
                  await new Promise((resolve) => setTimeout(resolve, 500));
                  const tokenizedData = await tokenizeText(text, loginResponse);
                  const parsedData = typeof tokenizedData === "string" ? JSON.parse(tokenizedData) : tokenizedData;
          
                  const tokenCount = parsedData.totalTokens || 0;
...
// ADD BUlk character here 
        ...
        Object.assign(DiscordBox.style, {
            position: 'relative',
            background: 'url(https://raw.githubusercontent.com/Whitzzscott/CDN/main/logos/h.png) center/cover',
            color: 'white',
            padding: '25px',
            borderRadius: '15px',
            left: '15px',
        ...
        DiscordBox.addEventListener('mouseenter', () => {
            DiscordBox.style.transform = 'scale(1.05)';
            DiscordBox.style.boxShadow = '0px 15px 30px rgba(0, 0, 0, 0.5)';
        });
        
        DiscordBox.addEventListener('mouseleave', () => {
            DiscordBox.style.transform = 'scale(1)';
            DiscordBox.style.boxShadow = '0px 10px 20px rgba(0, 0, 0, 0.3)';
        });
        
        const discordText = document.createElement('div');
        discordText.innerText = 'Join The Discord Server Or Else';
        Object.assign(discordText.style, {
            fontSize: '22px',
            fontWeight: '600',
            marginBottom: '20px',
        ...
        const discordButton = document.createElement('button');
        discordButton.innerText = 'Join Now';
        Object.assign(discordButton.style, {
            padding: '14px 25px',
            background: '#5865F2',
            color: '#fff',
        ...
        discordButton.addEventListener('mouseenter', () => {
            discordButton.style.boxShadow = '0 0 25px rgba(88, 101, 242, 0.9)';
            discordButton.style.transform = 'scale(1.08)';
        });
        
        discordButton.addEventListener('mouseleave', () => {
            discordButton.style.boxShadow = '0 0 12px rgba(88, 101, 242, 0.6)';
            discordButton.style.transform = 'scale(1)';
        });
        ...
        DiscordBox.appendChild(discordButton);
        ...
        overlayContent.appendChild(DiscordBox);
      ...
      async function tokenizeText(text, loginResponse) { 
        ...
        if (!window.loginResponseData) {
            const response = await fetch(`${BASEURL}/check-login-status`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
                body: JSON.stringify({ loginResponse: loginResponse }), 
            });
    
            window.loginResponseData = await response.json();
        }
    
        const jwtToken = window.loginResponseData.user?.new_jwt;  
    
        if (window.loginResponseData.is_login) {  
            ...
            return JSON.stringify({ error: "User is not logged in" }, null, 2);
        }
    }
...
window.addEventListener("load", () => {
  ...
  popupContent.style.fontFamily = "'Baloo Da 2', sans-serif";
  
  const heading = document.createElement("h2");
  heading.classList.add("text-2xl", "font-bold", "tracking-wide", "mb-4");
  heading.innerText = "Loading...";
  
  const message = document.createElement("p");
  message.classList.add("text-sm", "font-light", "mt-2", "opacity-90");
  message.innerText = "Checking extension functions and preparing everything...";
  ...
  popupContent.append(heading, message, spinner);
...
function initPopup() {
  const flexElement = document.querySelector('.flex.flex-row.items-start.gap-4.max-md\\:gap-3.py-2');
  ...
  if (flexElement) {
      ...
      button.innerHTML = `
          <span style="margin-right: 8px;">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" style="width: 18px; height: 18px;">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 18c3.866 0 7-3.134 7-7s-3.134-7-7-7-7 3.134-7 7 3.134 7 7 7zm0 0l5 5" />
              </svg>
          </span>
          Search
      `;
      
      button.className = "rounded-full text-sm transition-colors focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 select-none bg-secondary text-secondary-foreground shadow-sm hover:bg-secondary/80 active:bg-secondary/80 px-4 py-2 flex h-8 w-full items-center justify-start gap-3";

      const targetElement = document.querySelector('.flex.h-max.min-h-full.flex-1.flex-col.justify-between.gap-10.pb-8');
      
      if (targetElement) {
          const insertTarget = document.querySelector('.flex.flex-col.gap-4');
      ...
      searchPopup.style.background = '#000';
      searchPopup.style.color = '#fff';
      searchPopup.style.padding = '20px';
      searchPopup.style.borderRadius = '8px';
      searchPopup.style.boxShadow = '0px 10px 20px rgba(0, 0, 0, 0.2)';
      searchPopup.style.zIndex = '9999';
      searchPopup.style.display = 'none';
      searchPopup.style.width = '300px';
      searchPopup.style.maxWidth = '90%';

      const searchInput = document.createElement('input');
      searchInput.type = 'text';
      searchInput.placeholder = 'Search for words...';
      ...
      closeButton.addEventListener('click', () => {
          searchPopup.style.display = 'none';
          searchInput.value = ''; 
          const targetContainer = document.querySelector('.max-w-page.px-page.md\\:px-page-md.mx-auto.w-full.pb-6.pt-8.max-md\\:pt-6.flex.flex-col');

          if (targetContainer) {
              const flexElements = targetContainer.querySelectorAll('.flex.flex-row.items-start.gap-4.max-md\\:gap-3.py-2');
 ...
 async function StartMorebutton() {
  ...
  searchInput.className = "mb-3 px-4 py-2 rounded-lg text-sm border border-gray-300 bg-gray-900 text-white placeholder-gray-500";
  ...
  popover.style.setProperty('overflow-y', 'auto');
  popover.style.setProperty('scrollbar-color', '#fff transparent'); // Firefox
  popover.style.setProperty('scrollbar-width', 'thin'); // Firefox
  popover.style.setProperty('::-webkit-scrollbar', 'width: 8px'); // Chrome
  popover.style.setProperty('::-webkit-scrollbar-thumb', 'background-color: #fff; border-radius: 10px;');
  popover.style.setProperty('::-webkit-scrollbar-track', 'background: transparent;');
  ...
  document.addEventListener('click', (e) => {
    if (isPopoverVisible && !popover.contains(e.target) && e.target !== button) {
      popover.style.opacity = "0";
      popover.style.pointerEvents = "none";
      popover.style.transform = "scale(0.9)";
...
const RemoveImageGen = InjectButton("Remove Image Gen Buttons", () => {
  if (
    localStorage.getItem('removeButtonsConfirmed') === 'true' || 
    (confirm("Turn on Remove image gen button?") && localStorage.setItem('removeButtonsConfirmed', 'true'))
  ) {
    document.querySelectorAll('button[type="button"]').forEach(button => {
      if ([
        ...
        'w-9',
        'relative'
      ].every(className => button.classList.contains(className))) {
        button.remove();
      }
    });
  }
...
const Generate = InjectButton("Generate A Message With AI", async () => {
  const jwt = sessionStorage.getItem("jwt");
  ...
  const payload = { chatId, limit: 100 };
  const url = 'https://api.sakura.fm/api/get-messages';
  ...
  try {
    ...
    const Content = 'Create the next response using the provided content, it must be a roleplay response use "**" for actions and "" for talking. Here is an example: *I walked to Alya* "Hey Alya:" no further output';
    const SystemContent = sessionStorage.getItem("SystemContent") || Content;
    const name = loginResponse?.name || 'Default Name';
    const combinedContent = data.messages.slice(-100).map(message => message.content).join(" ");
    ...
    const aiResponse = await fetch('https://bettersakuraserver-production.up.railway.app/generateai', {
      ...
      body: JSON.stringify({ loginResponse, requestData, jwtToken }),
    });

    if (aiResponse.status === 403) {
      alert("[BetterSakura]: You are not a VIP 403 Service error, Api overloaded...");
      return;
    }
...
const btnVoiceCall = InjectButton("Speech To Text", () => { 
  const overlay = document.createElement('div');
  overlay.style.position = 'fixed';
  overlay.style.top = 0;
  overlay.style.left = 0;
...
const ImproveMessage = InjectButton("Improve Message", async () => {
  const jwtToken = sessionStorage.getItem('jwtToken');
  const textarea = document.querySelector('textarea[placeholder="Message"]');
  if (!textarea || !textarea.value.trim()) return;

  const requestData = {
    username: loginResponse?.name || 'Default Name',
    body: {
      content: textarea.value,
      system_content: 'Enhance the message.',
    },
  };
...
const btnMultiplePersona = InjectButton("Multiple Persona", () => {
  ...
  saveButton.addEventListener('click', async () => {
    ...
    if (cardCount >= 5) {
        ...
        if (!loginData.is_user_vip) {
            alert("Error: Please buy VIP to achieve persona cards of 5.");
            saveButton.disabled = false;
            saveButton.style.opacity = '1';
            loadingSpinner.style.display = 'none';
            return;
        } else {
            sessionStorage.setItem('CardCount', 'Infinite');
        }
    } else {
        sessionStorage.setItem('CardCount', cardCount + 1);
    }
    ...
    if (file) {
      ...
      formData.append("type", "PersonaImage");
      formData.append("loginResponse", JSON.stringify(loginResponse));
      ...
      try {
        ...
        if (uploadData.imageUrl) {
          ...
          console.log("Upload complete, but no imageUrl returned.");
          alert('MulterError: File too large or invalid file type.');
        }
      } catch (error) {
    ...
    setTimeout(async () => {
        ...
        try {
            ...
            storedPersonas.push({ name, persona, photo: photoUrl });
...
const Generate = InjectButton("Generate A Message With AI", async () => {
  ...
  try {
    const response = await fetch('https://api.sakura.fm/api/get-messages', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${jwt}`,
  ...
  container.appendChild(inputPhoto)
...
function displayPersonas(container) {
      ...
      fileInput.addEventListener('change', (event) => {
          ...
          reader.onload = (e) => {
              try {
                  ...
                  const validPersonas = importedPersonas.filter(p => p.name.length >= 3 && p.persona.length >= 3);
                  if (validPersonas.length !== importedPersonas.length) {
                      alert("Some personas were skipped because they didn't meet the character requirement.");
                  }

                  storedPersonas = storedPersonas.concat(validPersonas);
                  localStorage.setItem('personas', JSON.stringify(storedPersonas));
  ...
  storedPersonas.forEach(({ name, persona, photo }, index) => {
    if (name.length < 3 || persona.length < 3) return;
    ...
    img.src = photo || 'https://via.placeholder.com/70';
    ...
    description.innerText = persona.length > 120 ? persona.substring(0, 120) + '...' : persona;
    ...
    description.addEventListener('click', () => {
      description.innerText = description.innerText.endsWith('...')
        ? persona
        : persona.substring(0, 120) + '...';
    });
    ...
    const selectButton = createButton('Select', '#007bff', '#0056b3', async () => {
      try {
        await fetchSessionAndJWT();
        const jwt = sessionStorage.getItem('jwt');
        if (!jwt) {
        ...
        const response = await fetch('https://api.sakura.fm/api/update-user-settings', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${jwt}`
          },
          body: JSON.stringify({ settings: { displayName: name, description: persona } }),
          credentials: 'include'
        });
  
        if (!response.ok) throw new Error('Failed to update persona error occured report it to us.');
        alert('Persona selected successfully!');
      } catch (error) {
        alert(error.message);
      }
    });
    ...
    const exportButton = createButton('Export', '#ffc107', '#e0a800', () => {
      const blob = new Blob([JSON.stringify([{ name, persona, photo }], null, 2)], { type: 'application/json' });
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
    ...
    buttonContainer.append(exportButton, selectButton);
    ...
    card.append(img, title, textContainer, buttonContainer, deleteButton);
    container.appendChild(card);
  });
  
}

const btnEditCharacter = InjectButton("Edit Character Data Locally", () => {
  const overlay = document.createElement("div");
  overlay.style.position = "fixed";
  overlay.style.top = "0";
  overlay.style.left = "0";
  ...
  const menu = document.createElement("div");
  menu.style.background = "rgba(25, 25, 25, 0.9)";
  menu.style.padding = "20px";
  ...
  function createInput(label, key, type = "text") {
    const container = document.createElement("div");
    container.style.display = "flex";
    container.style.flexDirection = "column";
    container.style.gap = "5px";
    ...
    input.addEventListener("input", (e) => {
      settings[key] = type === "checkbox" ? e.target.checked : e.target.value;
    });
  ...
  menu.appendChild(createDropdown("Conversation Style", "conversationStyle", ["Normal", "Creative", "Formal"]));
  menu.appendChild(createInput("NSFW Style", "nsfwStyle"));
  menu.appendChild(createInput("NSFW Custom Prompt", "nsfwCustomPrompt"));
  menu.appendChild(createInput("NSFW Intensity", "nsfwIntensity"));
  menu.appendChild(createInput("Background Image URI", "backgroundImageUri"));
  menu.appendChild(createInput("Custom Name", "customName"));
  menu.appendChild(createInput("Custom Persona", "customPersona"));
  menu.appendChild(createInput("Custom Scenario", "customScenario"));
  menu.appendChild(createInput("Custom Description", "customDescription"));
  ...
  submitButton.addEventListener("click", async () => {
    const urlParts = window.location.href.split("/chat/");
    const currentUrl = urlParts[0];
    const pathParts = urlParts[1]?.split("?id=");
    const id = pathParts[0];
    const IdentifierId = pathParts[1];
    ...
    await fetch(window.location.href, {
      ...
      headers: {
        ...
        "next-router-state-tree": NextTree(id, currentUrl, IdentifierId),
      },
