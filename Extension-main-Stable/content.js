...
const checkLoginStatus = async () => {
    try {
        const response = await fetch('https://tiktoken-production.up.railway.app/check-login-status');
...
const loginStatusCheck = async () => {
    const responseDatav1 = await fetch('https://tiktoken-production.up.railway.app/check-login-status');
    ...
    if (isLoggedIn && isLoggedIn.is_login === true) {
        ...
        tokenizeButton.onclick = async () => {
            try {
                const loginCheckResponse = await fetch('https://tiktoken-production.up.railway.app/check-login-status', {
                    method: 'GET',
                    headers: {
        ...
        logoutButton.onclick = () => {
            ...
            yesButton.onclick = async () => {
                try {
                    const statusResponse = await fetch('https://tiktoken-production.up.railway.app/check-login-status');
                    ...
                    const response = await fetch('https://tiktoken-production.up.railway.app/logout', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        }
...
function extractCharacterData() {
    const characterContainers = document.querySelectorAll('.group.relative.flex.w-full.flex-col.bg-transparent, .group.relative.flex, .flex.w-full.flex-col.bg-transparent');
    const extractedData = [];
    ...
    if (characterContainers.length > 0) {
        characterContainers.forEach(character => {
            ...
            const lineClamp2 = character.querySelector('.text-muted-foreground.line-clamp-2');
            const lineClamp3 = character.querySelector('.text-muted-foreground.line-clamp-3');
            const lineClamp5 = character.querySelector('.text-muted-foreground.line-clamp-5');
            
            characterData.lineClamp2 = lineClamp2 ? lineClamp2.innerText : 'nan';
            characterData.lineClamp3 = lineClamp3 ? lineClamp3.innerText : 'nan';
            characterData.lineClamp5 = lineClamp5 ? lineClamp5.innerText : 'nan';

            const lineClamp3Elements = character.querySelectorAll('.text-muted-foreground.line-clamp-3');
            if (lineClamp3Elements.length > 0) {
                characterData.lineClamp3 = Array.from(lineClamp3Elements).map(el => el.innerText).join(', ');
            }

            const lineClamp5Elements = character.querySelectorAll('.text-muted-foreground.line-clamp-5');
            if (lineClamp5Elements.length > 0) {
                characterData.lineClamp5 = Array.from(lineClamp5Elements).map(el => el.innerText).join(', ');
            }

            const imageDiv = character.querySelector('.mx-auto.rounded-md.object-cover') || character.querySelector('[srcset]');
            if (imageDiv) {
                const srcset = imageDiv.getAttribute('srcset');
                characterData.srcset = srcset ? srcset : 'nan';
            } else {
                characterData.srcset = 'nan';
            }

            extractedData.push(characterData);
        });
        ...
        console.error('No matching character containers found');
    }
}
  ...
  clearInterval(intervalId);
        ...
        bugReportButton.onclick = () => {
            ...
            sendButton.onclick = async () => {
                if (input.value.trim()) {
                    try {
                        const response = await fetch('https://tiktoken-production.up.railway.app/api/send/bugrepot', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Authorization': 'Bearer SSS155'
        ...
        feedbackButton.onclick = () => {
            ...
            sendButton.onclick = async () => {
                if (input.value.trim()) {
                    try {
                        const response = await fetch('https://tiktoken-production.up.railway.app/api/send/feedback', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Authorization': 'Bearer SSS155'
        ...
        promptLibraryButton.onclick = async () => {
            try {
                const viewUrl = 'https://tiktoken-production.up.railway.app/prompts';
        ...
        async function submitPrompt(requestData) {
            ...
            try {
                const response = await fetch('https://tiktoken-production.up.railway.app/prompt_library', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': 'Bearer SSS155'
        ...
        async function tokenizeText(text) {
            ...
            const API_URL = 'https://tiktoken-production.up.railway.app/tokenize';
