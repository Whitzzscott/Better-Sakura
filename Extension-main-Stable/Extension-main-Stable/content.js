...
function fetchAndInsertContent() {
  ...
  button.addEventListener('click', async (e) => {
    ...
    const payload = { chatId, limit: 100 }; // Get last 100 messages
    const url = 'https://api.sakura.fm/api/get-messages';
    ...
    try {
      ...
      if (data && data.messages) {
        ...
        const combinedContent = messages.slice(-100).map(message => message.content).join(" ");
  ...
  saveButton.addEventListener('click', async () => {
    ...
    if (cardCount >= 5) {
        const response = await fetch(`${BASEURL}/check-login-status`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ loginResponse }) // Including loginResponse in the request body
        });
